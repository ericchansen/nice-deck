...
Inspect the source deck, assets, `brief.md`, `sources.json`,
...
- a clear question and supported answer
- evidence that actually supports the answer
- why that evidence changes a decision
- an honest caveat or uncertainty boundary
- claim status, reader-facing citations, and resolvable source IDs
...
recommendations stronger than the evidence. Notes may add depth but may not
supply missing slide logic.
...
gradient text, colored card stripes, tiny tracked kickers, and topic-reflex
imagery.

For data slides, verify the sanctioned ECharts SVG renderer, direct labels,
units, assumptions, takeaway, relevance, source IDs, deterministic capture
state, and useful no-hover default. Explain or remove legends, dashed reference
lines, and silent empty regions.
...
proofs approved through `deck-explore-direction`.

## 5. Verify delivery readiness
...
- canonical URLs and working links

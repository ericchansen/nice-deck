...
/**
 * Source-only plain-text editing. No runtime DOM and no HTML serialization.
...
const reviewHint = "Saved text requires content reconciliation and a fresh render/review. "
...
const unsupportedHint = "No source-backed text fields. Native slide text, table cells, notes, "
  + "citation labels and SVG labels are editable. Explicit locks, scripts and embedded application controls are not.";
...
function inspect(source) {
  ...
  return {
    public: {
      ...
      reviewHint: fields.length ? "Plain-text source editing only. Saved changes will require review." : unsupportedHint,
    },
    locations,
  };
}
...
function validateChanges(changes) {
  if (!Array.isArray(changes) || changes.length > 100) {
    throw new EditError("invalid-changes", "changes must be an array of at most 100 text edits.");
  }
  const ids = new Set();
  for (const change of changes) {
    if (!change || typeof change !== "object" || Array.isArray(change)
      || Object.keys(change).some((key) => !["id", "oldText", "text"].includes(key))
      || typeof change.id !== "string" || typeof change.oldText !== "string" || typeof change.text !== "string") {
      throw new EditError("invalid-changes", "Each change must contain only string id, oldText and text.");
    }
    if (ids.has(change.id)) throw new EditError("duplicate-change", "A field may be changed only once per request.");
    ids.add(change.id);
  }
}
    ...
    return {
      ...
      save(request) {
        ...
        if (!request || typeof request !== "object" || Array.isArray(request)
          ...
          return Promise.reject(new EditError("invalid-request", "Save requires a SHA-256 revision and text changes only."));
        }
        let changes;
        try {
          validateChanges(request.changes);
          changes = request.changes.map(({ id, oldText, text }) => ({ id, oldText, text }));
        } catch (cause) {

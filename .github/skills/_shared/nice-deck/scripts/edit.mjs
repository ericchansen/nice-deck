...
import { EditError, openEditableSource, patchEditableHtml, inspectEditableHtml } from "./edit-source.mjs";
...
export async function startEditorServer({ sourcePath, preview, onError = () => {} } = {}) {
  ...
  async function handleRequest(request, response) {
    ...
    try {
      ...
      } else if (pathname === "/api/draft" && request.method === "POST") {
        const body = await jsonBody(request);
        if (!body || Object.keys(body).some((key) => !["revision", "changes"].includes(key))) {
          throw error(400, "invalid-request", "Draft requires revision and bounded changes.");
        }
        const current = await coordinator.read();
        if (body.revision !== current.revision) throw error(409, "revision-conflict", "Source revision changed. Draft retained.");
        json(response, inspectEditableHtml(patchEditableHtml(current.html, body.changes)));
